...
DELETE FROM tokenized_all WHERE src IN
( 'BWV851'
, 'BWV852'
, 'BWV854'
, 'BWV860'
...
BEGIN
  FOR s, v IN
    ...
    ORDER BY src, vox
  LOOP
    CALL build_objects  (s,v);
    CALL detect_contexts(s,v);
    CALL extract_notes  (s,v);
  END LOOP;
END;
$$ LANGUAGE plpgsql;

\copy (TABLE notes) TO 'pgwtc-notes.csv' CSV HEADER
